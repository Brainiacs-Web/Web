...
confirmBtn.addEventListener("click", async function () {
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      // Add bonus EXP every 10 hours (600 minutes) for each 10-hour interval
      const totalHours = totalMinutes / 60;
      const bonusExp = Math.floor(totalHours / 10) * 5; // Add 5 EXP for every 10 hours
      const finalExp = updatedExp + bonusExp;

      // Update EXP in Firestore
      await updateDoc(userRef, { exp: finalExp });

      // Update UI
      updateUserInList(username, finalExp);
